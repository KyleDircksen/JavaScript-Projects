//creates an object too keep track of values
const Calculator = {
    //Displays the 0 on the screen
    Display_Value: '0',
    //this will hold the first operand for any expressions, we sit it to null for now
    First_Operand: null,
...
function Input_Digit(digit) {
    ...
    //we are checking to see if wait_second_operand is true and then set
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed, we convert the current number displayed on the screen
    //to a number and then store the result in Calculator.First_Operand if it doesn't already exist
    const Value_of_Input = parseFloat(Display_Value);
    //check if an operator already exists and if Wait_Second_operand is true, then updates
    ...
    if (First_Operand == null) {
        ...
        //if operator exists, property lookup is then performed for the operator in
...
keys.addEventListener('click', (event) => {
    //the target variable is an object that represents the element which was clicked
    const { target } = event;
    //if the element that was clicked on is nut a button, the function exits
